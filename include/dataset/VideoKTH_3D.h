...
{
	/**
	 * VideoKTH_3D loads KTH videos using pre-computed HOG frame data.
	...
	public:
		...
		/**
		 * Returns the bounding boxes for a given sample index and temporal frame.
		 * Used by HOG_Convolution3D to get pre-computed bounding boxes.
		 */
		static const std::map<std::string, VideoHOGData>& get_hog_data();

		/**
		 * Returns the sample mapping built during data loading.
		...
		/**
		 * Returns true if the loaded NPY metadata contains per-sample
		 * bboxes (full-frame mode). False for legacy crop mode.
		 */
		static bool has_bboxes();

		/**
		 * Returns (x, y, w, h) for the bbox of train sample `local_idx`
