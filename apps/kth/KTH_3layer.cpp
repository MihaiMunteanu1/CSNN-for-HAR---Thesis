...
int main(int argc, char **argv)
{
	...
	int epochs3              = env_int("CSNN_EPOCHS3", 80);
	...
	float th_mean3           = env_float("CSNN_TH_MEAN3", 5.0f);
	float th_dev3            = env_float("CSNN_TH_DEV3", 1.0f);
	float w_lr1              = env_float("CSNN_W_LR", 0.1f);
	float w_lr2              = env_float("CSNN_W_LR2", 0.05f);
	float w_lr3              = env_float("CSNN_W_LR3", 0.025f);
	...
	std::string suffix = "_s" + std::to_string(seed)
						...
						+ "_t3_" + std::to_string(t_obj3)
						+ "_f" + std::to_string(filter_h) + "x" + std::to_string(filter_w)
						+ "x" + std::to_string(filter_t);
	if (!tag.empty()) suffix += "_" + tag;

	std::filesystem::create_directories("result_3layer/" + eval_split + suffix);
	std::filesystem::create_directories("model_3layer/" + eval_split + suffix);

	Experiment<SparseIntermediateExecutionNew> experiment(
		argv, argc,
		"result_3layer/" + eval_split + suffix,
		"model_3layer/" + eval_split + suffix,
		"kth_3layer" + suffix,
		seed, true,
		false, false
	);

	std::cout << "=== KTH_3layer config ===" << std::endl;
	std::cout << "  seed         = " << seed << std::endl;
	std::cout << "  t_obj1       = " << t_obj1 << std::endl;
	std::cout << "  t_obj2       = " << t_obj2 << std::endl;
	std::cout << "  t_obj3       = " << t_obj3 << std::endl;
	std::cout << "  filter       = " << filter_h << "x" << filter_w << "x" << filter_t
			  << " (shared conv1/conv2/conv3)" << std::endl;
	std::cout << "  num_filters  = " << num_filters1 << " (conv1) / "
			  << num_filters2 << " (conv2) / " << num_filters3 << " (conv3)" << std::endl;
	std::cout << "  pool         = " << pool_sx << "x" << pool_sy << "x" << pool_st
			  << " (shared pool1/pool2/pool3)" << std::endl;
	...
	std::cout << "  epochs       = " << epochs1 << " (conv1) / " << epochs2
			  << " (conv2) / " << epochs3 << " (conv3)" << std::endl;
	std::cout << "  th_init1     = N(" << th_mean1 << ", " << th_dev1 << ")" << std::endl;
	std::cout << "  th_init2     = N(" << th_mean2 << ", " << th_dev2 << ")" << std::endl;
	std::cout << "  th_init3     = N(" << th_mean3 << ", " << th_dev3 << ")" << std::endl;
	std::cout << "  w_lr         = " << w_lr1 << " (conv1) / " << w_lr2
			  << " (conv2) / " << w_lr3 << " (conv3)" << std::endl;
	std::cout << "=========================" << std::endl;
	...
	std::cout << "  video_frames = " << video_frames
			  << " (runtime temporal T; expect chain " << video_frames
			  << "->conv1->conv2->conv3)" << std::endl;
	std::cout << "=========================" << std::endl;
	...
	auto &conv3 = experiment.push<layer::ConvolutionSampler3D>(
		num_filters3, filter_h, filter_w, filter_t, "", 1, 1, 2);
	conv3.set_name("conv3");
	conv3.parameter<bool>("draw").set(false);
	conv3.parameter<bool>("save_weights").set(false);
	conv3.parameter<bool>("save_random_start").set(false);
	conv3.parameter<bool>("log_spiking_neuron").set(false);
	conv3.parameter<bool>("inhibition").set(true);
	conv3.parameter<uint32_t>("epoch").set(static_cast<uint32_t>(epochs3));
	conv3.parameter<float>("annealing").set(0.95f);
	conv3.parameter<float>("min_th").set(1.0f);
	conv3.parameter<float>("t_obj").set(t_obj3);
	conv3.parameter<float>("lr_th").set(th_lr);
	conv3.parameter<size_t>("max_train_spikes").set(0);
	conv3.parameter<bool>("wta_infer").set(false);
	conv3.parameter<Tensor<float>>("w").distribution<distribution::Uniform>(0.0, 1.0);
	conv3.parameter<Tensor<float>>("th").distribution<distribution::Gaussian>(th_mean3, th_dev3);
	conv3.parameter<STDP>("stdp").set<stdp::Biological>(w_lr3, 0.1f);

	if (sampler_kind == "hog")
		conv3.parameter<Sampler>("sampler").set<sampler::HOGSampler3D>(4, 4);
	else
		conv3.parameter<Sampler>("sampler").set<sampler::RandomSampler3D>();

	auto &pool3 = experiment.push<layer::Pooling3D>(
		pool_sx, pool_sy, 1, pool_sx, pool_sy, 1);
	pool3.set_name("pool3");

	auto &conv3_out = experiment.output<TimeObjectiveOutput>(conv3, t_obj3);
	conv3_out.add_postprocessing<process::SumPooling>(2, 2);
