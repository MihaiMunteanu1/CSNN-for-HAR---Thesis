#!/usr/bin/env bash
# Launch a 3-layer OPTUNA tuning study inside a detached tmux session.
# Conv1 and conv2 are frozen at the per-sampler bests from the 1-/2-layer
# studies; only t_obj3 is tuned. See optuna/tune_csnn_optuna_3layer.py.
...
if [ -n "$STUDY_SUFFIX" ]; then
    STUDY_NAME="csnn_3layer_${SAMPLER}_${STUDY_SUFFIX}"
    SESSION_TAG="3layer_${SAMPLER}_${STUDY_SUFFIX}"
else
    STUDY_NAME="csnn_3layer_${SAMPLER}"
    SESSION_TAG="3layer_${SAMPLER}"
fi

BINARY="${CSNN_BUILD_DIR}/KTH_3layer"
if [ ! -x "$BINARY" ]; then
    echo "ERROR: binary not found at $BINARY"
    echo "  build it first: cmake --build $CSNN_BUILD_DIR --target KTH_3layer"
    exit 2
fi
...
CMD="python3 optuna/tune_csnn_optuna_3layer.py \
    --binary $BINARY \
    --data $DATA \
...
echo "  binary    = $BINARY"
echo "  data      = $DATA"
echo "  trials    = $N_TRIALS (seeds=$N_SEEDS, n_jobs=$N_JOBS, epochs=$EPOCHS1/$EPOCHS2/$EPOCHS3)"